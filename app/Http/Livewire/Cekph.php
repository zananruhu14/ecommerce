...
use App\Exports\testwwtpExport;
use App\Models\ph;
use Livewire\Component;
use Maatwebsite\Excel\Facades\Excel;

class Cekph extends Component
{
    public $start, $end;
    public $cod, $bod, $tds, $tss, $color, $do, $sv30, $qty, $temperatur, $shift, $tanggal, $status, $edit_id;
    ...
    public function render()
    {
        ...
        $cekph1 = ph::where('shift', 1)->where('status', 1)->whereBetween('tanggal', [$this->start, $this->end])->orderBy('tanggal', 'asc')->get();
        $cekph2 = ph::where('shift', 2)->where('status', 1)->whereBetween('tanggal', [$this->start, $this->end])->orderBy('tanggal', 'asc')->get();
        $group = ph::groupBy('tanggal')->where('status', 1)->whereBetween('tanggal', [$this->start, $this->end])->orderBy('tanggal', 'asc')->pluck('tanggal');
        return view('livewire.cekph', compact('cekph1', 'cekph2', 'group'))->extends('layout.main')->section('container');
    ...
    public function export()
    {
        ...
        return Excel::download(new testwwtpExport($this->start, $this->end), 'dailytest.xlsx');
    }
}