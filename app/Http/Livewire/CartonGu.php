...
class CartonGu extends Component
{
    public $cust = 'P6023-345496-008';
    public $search;
    public $stlye;
    public $idp;
    public $buyer;
...
");
        ...
        $cek = collect(json_decode(json_encode($hasil), true))->whereIn('_Cust_Symbol', $filter_buyer)->map(function ($item) {
            if (strpos($item['xSpec'], 'CM') !== false) {
                $spec = explode('*', trim($item['xSpec']));
                if (count($spec) == 3) {
        ...
        $grouped_stock = collect(json_decode(json_encode($hasil), true))->where('_Cust_Symbol', "GU")->groupBy('xSpec')->map(function ($stock) {
            return [
                'size' => $stock[0]['xSpec'],
        ...
        // dd($cek);
        return view('livewire.carton-gu', compact('items', 'gid', 'grouped_items', 'total', 'cek', 'grouped_stock'))->extends('layout.dashboard')->section('container');
    }
}
